-- =====================================================================
-- nounous.idf.immo — schéma de la base et règles d'accès
--
...
--
-- Principe directeur : une nounou ne peut voir que ses propres opportunités.
...
create table if not exists nounous (
  ...
  email       text not null,
  telephone   text,
  metier      text,              -- assistante maternelle agréée, garde d'enfants à domicile…
  commune     text,
...
create table if not exists opportunites (
  ...
  nounou_id       uuid not null references nounous(id) on delete restrict,
  ...
  type_bien       text,
  avancement      text,
  lien_famille    text,               -- « je garde leur enfant », « au square »…
  details         text,
  ...
  numero_transmis    boolean not null default false,  -- la nounou lui a donné notre numéro
  ...
  constraint accord_obligatoire check (
    famille_tel is null or accord_atteste = true
  )
);

create index if not exists opportunites_cle_bien   on opportunites (cle_bien);
create index if not exists opportunites_nounou    on opportunites (nounou_id, partagee_le desc);
...
-- Tout contact effectif avec la famille relance les 24 mois.
...
create index if not exists primes_nounou on primes (nounou_id, cree_le desc);

-- Cumul annuel versé à une nounou : sert à l'informer avant que la question
...
returns integer language sql stable as $$
  ...
  from primes
  where nounou_id = g and statut = 'versee'
...
language sql stable security definer set search_path = public as $$
  ...
  from opportunites o
  join nounous g on g.id = o.nounou_id
  where o.cle_bien = cle
  order by o.partagee_le asc;
$$;
...
-- --- nounous : chacun sa fiche ---
drop policy if exists nounou_lit_sa_fiche on nounous;
create policy nounou_lit_sa_fiche on nounous
  for select using (id = auth.uid() or est_admin());

drop policy if exists nounou_cree_sa_fiche on nounous;
create policy nounou_cree_sa_fiche on nounous
  for insert with check (id = auth.uid());

drop policy if exists nounou_modifie_sa_fiche on nounous;
create policy nounou_modifie_sa_fiche on nounous
...
-- --- opportunités ---
-- La nounou voit les siennes et peut en créer. Il ne peut ni les modifier
-- ni les supprimer : le statut appartient au conseiller, et l'horodatage
-- de partage doit rester incontestable.
drop policy if exists nounou_lit_ses_opportunites on opportunites;
create policy nounou_lit_ses_opportunites on opportunites
  for select using (nounou_id = auth.uid() or est_admin());

drop policy if exists nounou_partage on opportunites;
create policy nounou_partage on opportunites
  for insert with check (nounou_id = auth.uid());
...
create policy lecture_journal on evenements
  for select using (
    est_admin() or exists (
      ...
      where o.id = evenements.opportunite_id and o.nounou_id = auth.uid()
    )
  );
...
-- --- primes ---
drop policy if exists nounou_lit_ses_primes on primes;
create policy nounou_lit_ses_primes on primes
  for select using (nounou_id = auth.uid() or est_admin());
...
grant select, insert, update on nounous     to authenticated;
...
-- rapprocher() reste hors de portée : elle révèle l'identité des nounous.
...
--
--   b) Vérifier que le verrou tient. Connectée en tant que nounou de test,
--      cette requête doit renvoyer zéro ligne :
--
--        select * from opportunites where nounou_id <> auth.uid();
--
